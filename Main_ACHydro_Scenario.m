...
%% -----------------------------------------------------------------------
% 9. CATCHMENT TRANSPIRATION COEFFICIENT (KC) IMPACT 
%------------------------------------------------------------------------
...
% 
% % Summary of annual discharge (median and relative changes to median)
...
% Row1=[Q_MTFyearstats(2,1),min(Q_MTFyearDeltastats(2,2:end))*100,median(Q_MTFyearDeltastats(2,2:end))*100,max(Q_MTFyearDeltastats(2,2:end))*100];
% Row2=[Q_MBFyearstats(2,1),min(Q_MBFyearDeltastats(2,2:end))*100,median(Q_MBFyearDeltastats(2,2:end))*100,max(Q_MBFyearDeltastats(2,2:end))*100];
% Row3=[Q_MIFyearstats(2,1),min(Q_MIFyearDeltastats(2,2:end))*100,median(Q_MIFyearDeltastats(2,2:end))*100,max(Q_MIFyearDeltastats(2,2:end))*100];
% Row4=[Q_MOFyearstats(2,1),min(Q_MOFyearDeltastats(2,2:end))*100,median(Q_MOFyearDeltastats(2,2:end))*100,max(Q_MOFyearDeltastats(2,2:end))*100];
% DataMatrix=[Row1;Row2;Row3;Row4];
% 
% xlswrite(filename,HeadersColumns,'Discharge','A1');
% xlswrite(filename,HeadersRows,'Discharge','A2');
% xlswrite(filename,DataMatrix,'Discharge','B2');
%     
% % Summary of Crop development and production parameters (median and absolute changes to median)
% 
% name={'Maize','WinterWheat','Sugarbeet','Potato','Pea'};
...
%         HeadersRows={name(1,c);'Seasonal crop yield (ton/ha)';'WPET (kg/m³)';'LGPact (days)';'TSI (%)';'DSI (%)'};
...
%         row2=[WPstats{2,c}(2,1),
%         row3=[LGPactstats{2,c}(2,1),
%         row4=[DSIstats{2,c}(2,1),
%         row5=[TSIstats{2,c}(2,1),
%         DataMatrix=[Row1;Row2,Row3;Row4;Row5];
% 
%         xlswrite(filename,HeadersColumns,'CropVar',[letters2(g) num2str(2)]);
%         xlswrite(filename,HeadersRows,'CropVar',[letters(g) num2str(2)]);
%         xlswrite(filename,DataMatrix,'CropVar',[letters2(g) num2str(3)]);
% 
%     end
% end
